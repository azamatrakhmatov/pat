...
function updateResponseIndicators() {
    ...
    questionsToAsk.forEach((question, index) => {
        ...
        if (userResponses[index] === question.correct) {
            ...
            span.innerText = `Вопрос ${index + 1}: Правелньй ответ`;
        } else if (userResponses[index] !== undefined) {
            span.style.color = "red";
            span.innerText = `Вопрос ${index + 1}: Не правелньй ответ`;
        }
...
function showResults() {
    ...
    resultDiv.innerHTML = `<div style="color: ${resultColor}; font-weight: bold;">Резултат: ${correctAnswers}/${questionsToAsk.length} (${((correctAnswers / questionsToAsk.length) * 100).toFixed(2)}%)<br>${resultMessage}</div><br>`;

    resultDiv.innerHTML += "НЕПРАВИЛЬНЫЕ ОТВЕТЫ:";
    questionsToAsk.forEach((question, index) => {
        if (userResponses[index] !== question.correct) {

            resultDiv.innerHTML += `<br><strong style="color: red;">Вопрос: ${question.question}, Ваш ответ: ${question.answers[userResponses[index]]}, прав/ответ: ${question.answers[question.correct]}</strong>`;
        }
    });
