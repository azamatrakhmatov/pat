const questions = [
        ...
        { number: 1, question: "В каком городе находится Кремль?", answers: ["Москва", "Санкт-Петербург", "Казань"], correct: 0 },
...
function loadQuestion() {
    ...
    nextButton.innerText = "Перейти к следующему вопросу";
...
function showResults() {
    ...
    let resultMessage = correctAnswers / questions.length < 0.6 ? 
        "Не удалось пройти тест, попробуйте еще раз" : 
        "Тест прошел успешно";

    const resultColor = correctAnswers / questions.length < 0.6 ? "red" : "green";
    resultDiv.innerHTML = `<div style="color: ${resultColor}; font-weight: bold;">Natijangiz: ${correctAnswers}/${questions.length} (${((correctAnswers / questions.length) * 100).toFixed(2)}%)<br>${resultMessage}</div><br>`;

    resultDiv.innerHTML += "НЕПРАВИЛЬНЫЕ ОТВЕТЫ:";
    questions.forEach((question, index) => {
        if (userResponses[index] !== question.correct) {
            resultDiv.innerHTML += `<br><strong style="color: red;">Вопрос: ${question.question}, Ваш ответ: ${question.answers[userResponses[index]]}, Правелньй ответ:${question.answers[question.correct]}</strong>`;
        }
    });

    const restartButton = document.createElement("button");
    restartButton.innerText = "Перезапуск теста";
...
function restartQuiz() {
    // Barcha o'zgaruvchilarni tozalash, lekin savollarni saqlab qolish
    currentQuestion = 0;
    correctAnswers = 0;
    userResponses = [];

    const answersElement = document.getElementById("answers");
    answersElement.innerHTML = ""; // Oldingi javoblarni o'chirish
    
    const indicatorDiv = document.getElementById("indicators");
    indicatorDiv.innerHTML = ""; // Oldingi indikatsiyalarni o'chirish
    
    const resultDiv = document.querySelector("div"); // Natija divini olish
    if (resultDiv) document.body.removeChild(resultDiv); // Agar result div mavjud bo'lsa, olib tashlash
    
    loadQuestion(); // Testni boshidan yuklash
}
